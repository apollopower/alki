...
def test_llama_model(repo_id: str, filename: str):
    ...
    try:
        ...
        test_prompt = "Give a short history of Alki beach in Seattle."
        logger.info(f"Test prompt: '{test_prompt}'")

        # Generate response with basic parameters
        response = model(
            test_prompt, max_tokens=50, temperature=0.7, top_p=0.9, echo=False
        )
...
Examples:
  ...
  %(prog)s "Qwen/Qwen2-0.5B-Instruct-GGUF" "*q8_0.gguf"
    ...
    parser.add_argument(
        "repo_id", help="HuggingFace repository ID (e.g., 'Qwen/Qwen3-0.6B-GGUF')"
    )
